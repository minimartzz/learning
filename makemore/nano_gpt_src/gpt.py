...
class Block(nn.Module):
  """
  Combines communication (attention) with computation (ffw) 
  """
  def __init__(self, n_embd, n_head):
    """
    Args:
        n_embd (int): Embedding dimension
        n_head (int): Number of heads to split the embedding dimension into
    """
    super().__init__()
  ...
  def forward(self, x):
    x = x + self.sa(self.ln1(x)) # x is added back in to represent residual connections
